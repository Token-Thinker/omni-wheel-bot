//! I2C Devices Module
//! This module manages I2C-connected devices, including motor control and IMU integration.
...
use icm42670::{Address as ImuAddress, Error as ImuError, Icm42670, PowerMode};
...
pub enum I2CCommand {
    ...
    O { d: f32, s: f32, rs: f32, o: Option<f32> },
    ...
    E: core::fmt::Debug,
{
    ...
    pub fn new(
        ...
        pwm.set_all_on_off(&[0; 16], &[0x0FFF; 16]).map_err(DeviceError::PwmError)?;
    ...
    fn apply_wheel_speeds(&mut self, wheel_speeds: &[f32]) -> Result<(), DeviceError<E>> {
        ...
        for (i, &(phase_channel, enable_channel)) in self.motor_channels.iter().enumerate() {
            ...
            self.pwm.set_channel_on_off(phase_channel, 0, if direction { 0 } else { MAX_DUTY })
                .map_err(DeviceError::PwmError)?;
            self.pwm.set_channel_on_off(enable_channel, 0, (speed * MAX_DUTY as f32) as u16)
                .map_err(DeviceError::PwmError)?;
        }
        Ok(())
    }
    ...
    pub fn enable(&mut self) -> Result<(), DeviceError<E>> {
        ...
        self.imu.set_power_mode(PowerMode::SixAxisLowNoise).map_err(DeviceError::ImuError)
    }
